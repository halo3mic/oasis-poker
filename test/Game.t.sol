// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.9;

import {Test, console2} from "lib/forge-std/src/Test.sol";
import "../contracts/Poker.sol";


interface ICheat {
    function etch(address who, bytes calldata code) external;
    function deal(address who, uint256 newBalance) external;
}

contract RandomBytes {

    function randomBytes(uint nbytes) public view returns (bytes memory result) {
        bytes memory data = hex"";
        assembly {
            result := mload(0x40)
            mstore(result, 52)
            for { let i := 0 } lt(i, 52) { i := add(i, 32) } {
                mstore(add(result, add(0x20, i)), mload(add(data, add(0x20, i))))
            }
            mstore(0x40, add(result, 0x60))
        }
    }

    fallback() external {
        (uint numBytes, bytes memory _pers) = abi.decode(msg.data, (uint, bytes));
        bytes memory rand = randomBytes(numBytes);

        assembly {
            let location := rand
            let length := mload(rand)
            return(add(location, 0x20), length)
        }
    }

}


contract Player {
    string public label;
    uint gameId;
    Poker target;

    constructor(address _target, uint _gameId, string memory _label) {
        target = Poker(_target);
        gameId = _gameId;
        label = _label;
    }

    function buyIn(uint amount) public {
        target.buyIn{ value: amount }(gameId);
    }

    function peekCards() public view returns (string memory, string memory) {
        return target.peek(gameId);
    }

    function play(PlayerAction action) public {
        target.play(gameId, action);
    }

}

contract GameTest is Test {
    using DeckUtils for Deck;
    using CardUtils for Card;
    using GameUtils for Game;

    ICheat cheat = ICheat(0x7109709ECfa91a80626fF3989D68f67F5b1DD12D);
    address constant RANDOM_BYTES = 0x0100000000000000000000000000000000000001;
    Poker public poker;

    function setUp() public {
        address rb = address(new RandomBytes());
        cheat.etch(RANDOM_BYTES, rb.code);
    }

    function newPlayer(string memory label, uint gameId) public returns (Player) {
        Player p = new Player(address(poker), gameId, label);
        cheat.deal(address(p), 100);
        return p;
    }

    function testGame() public {
        poker = new Poker();
        uint gameId = poker.createGame();

        Player player1 = newPlayer("Player 1", gameId);
        Player player2 = newPlayer("Player 2", gameId);
        Player player3 = newPlayer("Player 3", gameId);

        player1.buyIn(10);
        player2.buyIn(10);
        player3.buyIn(10);

        console2.log("Player 1 cards: ");
        (string memory c0, string memory c1) = player1.peekCards();
        console2.log(c0, c1);

        console2.log("Player 2 cards: ");
        (c0, c1) = player2.peekCards();
        console2.log(c0, c1);

        console2.log("Player 3 cards: ");
        (c0, c1) = player3.peekCards();
        console2.log(c0, c1);

        for (uint i = 0; i < 3; i++) {
            player1.play(PlayerAction.Check);
            player2.play(PlayerAction.Check);
            player3.play(PlayerAction.Check);
            string memory ccards = poker.getCommunityCards(gameId);
            console2.log("Community cards: ", ccards);
        }

        // todo: check game is closed

    }

}
